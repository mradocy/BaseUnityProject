// File copied from UserDataStore_PlayerPrefs, with PlayerPrefs replaced with ControlsPrefs.
...
namespace Core.Unity.RewiredExtensions {
    ...
    /// <summary>
    /// Class for saving data to ControlsPrefs. Add this as a component to your Rewired Input Manager to save and load data automatically to ControlsPrefs.
    /// Copy this class and customize it to your needs to create a new custom data storage system.
    /// </summary>
    public class UserDataStore_PrefsFile : UserDataStore {

        private const string thisScriptName = "UserDataStore_PrefsFile";
        private const string logPrefix = "Rewired: ";
        private const string editorLoadedMessage = "\n***IMPORTANT:*** Changes made to the Rewired Input Manager configuration after the last time XML data was saved WILL NOT be used because the loaded old saved data has overwritten these values. If you change something in the Rewired Input Manager such as a Joystick Map or Input Behavior settings, you will not see these changes reflected in the current configuration. Clear ControlsPrefs using the inspector option on the UserDataStore_ControlsPrefs component.";
        private const string controlsPrefsKeySuffix_controllerAssignments = "ControllerAssignments";
...
#if UNITY_4_6_PLUS
        [Tooltip("The ControlsPrefs key prefix. Change this to change how keys are stored in ControlsPrefs. Changing this will make saved data already stored with the old key no longer accessible.")]
#endif
        private const string controlsPrefsKeyPrefix = "RewiredSaveData";
        ...
        private string controlsPrefsKey_controllerAssignments { get { return string.Format("{0}_{1}", controlsPrefsKeyPrefix, controlsPrefsKeySuffix_controllerAssignments); } }
...
#if UNITY_EDITOR
            Debug.Log(logPrefix + thisScriptName + $" saved all user data to \"{ControlsPrefs.GetFilePath()}\".");
#endif
        }
...
#if UNITY_EDITOR
            Debug.Log(logPrefix + thisScriptName + " saved " + controllerType + " " + controllerId + " data for Player " + playerId + $" to \"{ControlsPrefs.GetFilePath()}\".");
#endif
        }
...
#if UNITY_EDITOR
            Debug.Log(logPrefix + thisScriptName + " saved " + controllerType + " " + controllerId + $" data to \"{ControlsPrefs.GetFilePath()}\".");
#endif
        }
...
#if UNITY_EDITOR
            Debug.Log(logPrefix + thisScriptName + " saved all user data for Player " + playerId + $" to \"{ControlsPrefs.GetFilePath()}\".");
#endif
        }
...
#if UNITY_EDITOR
            Debug.Log(logPrefix + thisScriptName + " saved Input Behavior data for Player " + playerId + $" to \"{ControlsPrefs.GetFilePath()}\".");
#endif
        }
...
#if UNITY_EDITOR
            if (count > 0) Debug.LogWarning(logPrefix + thisScriptName + $" loaded all user data from \"{ControlsPrefs.GetFilePath()}\". " + editorLoadedMessage);
#endif
        }
...
#if UNITY_EDITOR
            if (count > 0) Debug.LogWarning(logPrefix + thisScriptName + " loaded user data for " + controllerType + " " + controllerId + " for Player " + playerId + $" from \"{ControlsPrefs.GetFilePath()}\". " + editorLoadedMessage);
#endif
        }
...
#if UNITY_EDITOR
            if (count > 0) Debug.LogWarning(logPrefix + thisScriptName + " loaded user data for " + controllerType + " " + controllerId + $" from \"{ControlsPrefs.GetFilePath()}\". " + editorLoadedMessage);
#endif
        }
...
#if UNITY_EDITOR
            if (count > 0) Debug.LogWarning(logPrefix + thisScriptName + " loaded Player + " + playerId + $" user data from \"{ControlsPrefs.GetFilePath()}\". " + editorLoadedMessage);
#endif
        }
...
#if UNITY_EDITOR
            if (count > 0) Debug.LogWarning(logPrefix + thisScriptName + " loaded Player + " + playerId + $" InputBehavior data from \"{ControlsPrefs.GetFilePath()}\". " + editorLoadedMessage);
#endif
        }
...
#if UNITY_EDITOR
                Debug.LogWarning(logPrefix + thisScriptName + " loaded controller assignments from ControlsPrefs.");
#endif
            } catch {
#if UNITY_EDITOR
                Debug.LogError(logPrefix + thisScriptName + " encountered an error loading controller assignments from ControlsPrefs.");
#endif
            }
...
#if UNITY_EDITOR
                Debug.LogError(logPrefix + thisScriptName + " encountered an error loading keyboard and/or mouse assignments from ControlsPrefs.");
#endif
            }
...
#if UNITY_EDITOR
                Debug.LogError(logPrefix + thisScriptName + " encountered an error loading joystick assignments from ControlsPrefs.");
#endif
            }
        ...
        private ControllerAssignmentSaveInfo LoadControllerAssignmentData() {
            try {
                ...
                if (!ControlsPrefs.HasKey(controlsPrefsKey_controllerAssignments)) return null;

                // Load save data from the registry
                string json = ControlsPrefs.GetString(controlsPrefsKey_controllerAssignments);
...
#if UNITY_EDITOR
                Debug.LogWarning(logPrefix + thisScriptName + " loaded joystick assignments from ControlsPrefs.");
#endif
            }
        ...
        private void SaveJoystickCalibrationData(Joystick joystick) {
            ...
            string key = GetJoystickCalibrationMapControlsPrefsKey(joystick);
            ControlsPrefs.SetString(key, saveData.map.ToXmlString()); // save the map to player prefs in XML format
        }
        ...
        private void SaveControllerMap(Player player, ControllerMap controllerMap) {
            ...
            string key = GetControllerMapControlsPrefsKey(player, controllerMap.controller.identifier, controllerMap.categoryId, controllerMap.layoutId, controllerMapPPKeyVersion);
            ControlsPrefs.SetString(key, controllerMap.ToXmlString()); // save the map to player prefs in XML format
            ...
            key = GetControllerMapKnownActionIdsControlsPrefsKey(player, controllerMap.controller.identifier, controllerMap.categoryId, controllerMap.layoutId, controllerMapPPKeyVersion);
            ControlsPrefs.SetString(key, allActionIdsString);
        }
        ...
        private void SaveInputBehaviorNow(Player player, InputBehavior inputBehavior) {
            ...
            string key = GetInputBehaviorControlsPrefsKey(player, inputBehavior.id);
            ControlsPrefs.SetString(key, inputBehavior.ToXmlString()); // save the behavior to player prefs in XML format
        }
        ...
        private bool SaveControllerAssignments() {
            try {
                ...
                // Save to ControlsPrefs
                ControlsPrefs.SetString(controlsPrefsKey_controllerAssignments, JsonWriter.ToJson(allPlayerData));
                ControlsPrefs.Save();

#if UNITY_EDITOR
                Debug.Log(logPrefix + thisScriptName + " saved controller assignments to ControlsPrefs.");
#endif
            } catch {
#if UNITY_EDITOR
                Debug.LogError(logPrefix + thisScriptName + " encountered an error saving controller assignments to ControlsPrefs.");
#endif
            }
            return true;
        }
        ...
        private bool ControllerAssignmentSaveDataExists() {
            // Check if there is any data saved
            if (!ControlsPrefs.HasKey(controlsPrefsKey_controllerAssignments)) return false;
        ...
        #region ControlsPrefs Methods

        private string GetBaseControlsPrefsKey(Player player) {
            string key = controlsPrefsKeyPrefix;
        ...
        private string GetControllerMapControlsPrefsKey(Player player, ControllerIdentifier controllerIdentifier, int categoryId, int layoutId, int ppKeyVersion) {
            // Create a player prefs key like a web querystring so we can search for player prefs key when loading maps
            string key = GetBaseControlsPrefsKey(player);
            key += "|dataType=ControllerMap";
            key += GetControllerMapControlsPrefsKeyCommonSuffix(player, controllerIdentifier, categoryId, layoutId, ppKeyVersion);
            return key;
        }

        private string GetControllerMapKnownActionIdsControlsPrefsKey(Player player, ControllerIdentifier controllerIdentifier, int categoryId, int layoutId, int ppKeyVersion) {
            // Create a player prefs key like a web querystring so we can search for player prefs key when loading maps
            string key = GetBaseControlsPrefsKey(player);
        ...
        private static string GetControllerMapControlsPrefsKeyCommonSuffix(Player player, ControllerIdentifier controllerIdentifier, int categoryId, int layoutId, int ppKeyVersion) {
            string key = "";
        ...
        private string GetJoystickCalibrationMapControlsPrefsKey(Joystick joystick) {
            // Create a player prefs key like a web querystring so we can search for player prefs key when loading maps
            string key = controlsPrefsKeyPrefix;
        ...
        private string GetInputBehaviorControlsPrefsKey(Player player, int inputBehaviorId) {
            // Create a player prefs key like a web querystring so we can search for player prefs key when loading maps
            string key = GetBaseControlsPrefsKey(player);
        ...
        private string GetControllerMapXml(Player player, ControllerIdentifier controllerIdentifier, int categoryId, int layoutId) {
            ...
            for (int i = controllerMapPPKeyVersion; i >= 0; i--) {
                key = GetControllerMapControlsPrefsKey(player, controllerIdentifier, categoryId, layoutId, i);
                if (ControlsPrefs.HasKey(key)) return ControlsPrefs.GetString(key); // return the data
            }
            return null;
        }
        ...
        private List<int> GetControllerMapKnownActionIds(Player player, ControllerIdentifier controllerIdentifier, int categoryId, int layoutId) {
            ...
            // Must try many times because of new additions in various versions
            for (int i = controllerMapPPKeyVersion; i >= 0; i--) {
                key = GetControllerMapKnownActionIdsControlsPrefsKey(player, controllerIdentifier, categoryId, layoutId, i);
                if (ControlsPrefs.HasKey(key)) {
                    found = true;
                    break;
                }
            }
            ...
            string data = ControlsPrefs.GetString(key);
        ...
        private string GetJoystickCalibrationMapXml(Joystick joystick) {
            string key = GetJoystickCalibrationMapControlsPrefsKey(joystick);
            if (!ControlsPrefs.HasKey(key)) return string.Empty; // key does not exist
            return ControlsPrefs.GetString(key); // return the data
        }

        private string GetInputBehaviorXml(Player player, int id) {
            string key = GetInputBehaviorControlsPrefsKey(player, id);
            if (!ControlsPrefs.HasKey(key)) return string.Empty; // key does not exist
            return ControlsPrefs.GetString(key); // return the data
        }
